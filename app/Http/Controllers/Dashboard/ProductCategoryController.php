...
class ProductCategoryController extends Controller
{
    ...
    public function store(StoreProductCategory $request)
    {
        ...
        if (isset($inputs['image'])) {
            $inputs['image'] = uploadImage($inputs['image'], config('path.CATEGORY_PATH'), $inputs['ar']['title']);
        }
        // dd($inputs);
    ...
    public function update(StoreProductCategory $request, $id)
    {
        ...
        $resource = $this->model->findOrFail($id);
        if (isset($inputs['image'])) {
            $inputs['image'] = uploadImage($inputs['image'], config('path.CATEGORY_PATH'), $inputs['ar']['title'], $resource->image);
        }
