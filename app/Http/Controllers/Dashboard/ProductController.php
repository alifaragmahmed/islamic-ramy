...
class ProductController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('dashboard.products.index', [
            'data'       => $this->model->filter()->orderBy('sort', 'asc')->paginate(20),
    ...
    public function store(StoreProduct $request)
    {
        ...
        if (isset($inputs['image'])) {
            $inputs['image'] = uploadImage($inputs['image'], config('path.PRODUCTS_PATH'), $inputs['ar']['title']);
        }
        ...
        if ($request->images) {
            foreach ($request->images as $image) {
                ...
                $item = uploadImage($image, config('path.PRODUCTS_PATH') . 'images/', $inputs['ar']['title']);
    ...
    public function update(StoreProduct $request, $id)
    {
        ...
        $resource = $this->model->findOrFail($id);
        if (isset($inputs['image'])) {
            $inputs['image'] = uploadImage($inputs['image'], config('path.PRODUCTS_PATH'), $inputs['ar']['title'], $resource->image);
        }
        if (isset($inputs['project_logo'])) {
    ...
    {
        $inputs = $request->only('id', 'is_active');
