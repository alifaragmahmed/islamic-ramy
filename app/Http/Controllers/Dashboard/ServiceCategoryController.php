...
class ServiceCategoryController extends Controller
{
    ...
    public function store(StoreServiceCategory $request)
    {
        ...
        if (isset($inputs['image'])) {
            $inputs['image'] = uploadImage($inputs['image'], config('path.SER_CAT_PATH'), $inputs['ar']['title']);
        }
        // dd($inputs);
    ...
    public function update(StoreServiceCategory $request, $id)
    {
        ...
        $resource = $this->model->findOrFail($id);
        if (isset($inputs['image'])) {
            $inputs['image'] = uploadImage($inputs['image'], config('path.SER_CAT_PATH'), $inputs['ar']['title'], $resource->image);
        }
