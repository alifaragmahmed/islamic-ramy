...
@php
    $title = __('lang.video_categories');
@endphp
    ...
    <div class="row pt-2">
        <div class="col-md-12">
            @component('dashboard.layouts.includes.card')
                @slot('tool')
                    <a data-href="{{ route('admin.video-category.create') }}" data-container=".table-modal"
                            ...
                            @if (isset($data) && count($data) > 0)
                                @foreach ($data as $item)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($item->icon)
                                                <img src="{{ asset($item->icon) }}" class="border-8" width="40px" height="40px"
                                                    alt="">
                                            @else
                                                <span class="badge bg-secondary">{{ __('lang.no_icon') }}</span>
                                            @endif
                                        </td>

                                        <td>
                                            <a data-href="{{ route('admin.video-category.edit', $item->id) }}" data-container=".table-modal"
                                                class="btn btn-modal text-white btn-primary btn-sm"><i class="bx bx-edit"></i></a>
                                            <a href="{{ route('admin.video-category.delete', $item->id) }}"
                                                class="btn btn-danger sw-alert btn-sm"><i class="bx bx-trash"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
