...
<div class="modal-dialog modal-dialog-centered">
    <div class="modal-content ">
        ...
        <div class="modal-body">
            <form class="form"
                action="{{ $resource->id ? route('admin.customer_opinions.update', $resource->id) : route('admin.customer_opinions.store') }}"
                method="post" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-12  ">

                        {!! Form::label('image', __('lang.image') . ' 90 * 90', ['class' => 'form-label']) !!} <br>
                    ...
                    @foreach (config('translatable.locales') as $key => $locale)
                        <div class="col-md-6 pt-2">
                            <div class="form-group">
                                <label for="name">
                                    {{ __('lang.' . $locale . '.sub_title') }}
                                </label>
                                {!! Form::text("{$locale}[sub_title]", old("{$locale}[sub_title]", optional($resource->translate($locale))->sub_title), [
                                    'class' => 'form-control',
                                ]) !!}
                            </div>
                        </div>
